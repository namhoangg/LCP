package com.lcp.invoice.repository.custom.impl;

import com.lcp.common.EntityBase_;
import com.lcp.invoice.dto.DebitNoteListRequest;
import com.lcp.invoice.entity.DebitNote;
import com.lcp.invoice.entity.DebitNote_;
import com.lcp.invoice.repository.custom.DebitNoteRepositoryCustom;
import com.lcp.util.PageUtil;
import com.lcp.util.QueryUtil;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageImpl;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Component;

import javax.persistence.EntityManager;
import javax.persistence.TypedQuery;
import javax.persistence.criteria.CriteriaBuilder;
import javax.persistence.criteria.CriteriaQuery;
import javax.persistence.criteria.Predicate;
import javax.persistence.criteria.Root;
import java.util.ArrayList;
import java.util.List;

@Component
public class DebitNoteRepositoryCustomImpl implements DebitNoteRepositoryCustom {
    @Autowired
    EntityManager entityManager;

     @Override
     public Page<DebitNote> list(DebitNoteListRequest request) {
         CriteriaBuilder builder = entityManager.getCriteriaBuilder();
         CriteriaQuery<DebitNote> query = builder.createQuery(DebitNote.class);
         Root<DebitNote> root = query.from(DebitNote.class);
         query.select(root).distinct(true);
         List<Predicate> predicates = new ArrayList<>();

         if (request.getShipmentId() != null) {
             predicates.add(builder.equal(root.get(DebitNote_.SHIPMENT_ID), request.getShipmentId()));
         }

         if (request.getStatus() != null) {
             predicates.add(builder.equal(root.get(EntityBase_.STATUS), request.getStatus()));
         }

         query.where(predicates.toArray(new Predicate[]{}));
         query.orderBy(PageUtil.generateOrder(request, builder, root, EntityBase_.UPDATED_AT));

         TypedQuery<DebitNote> typedQuery = entityManager.createQuery(query);
         var total = QueryUtil.count(builder, query, root);
         Pageable pageable = request.getPageable();
         List<DebitNote> list = typedQuery.setFirstResult((int) pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
         return new PageImpl<>(list, pageable, total);
     }
}
